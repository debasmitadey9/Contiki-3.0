...
#include "ecc.h"
#include "ecc_sha1.h"
//#include "../../../../Projects/contiki-2.7/eglibc/libc/sysdeps/ieee754/dbl-64/mydefs.h"
//#include "lib/rand.h"
#include <stdlib.h>
...
shamir_init ( point_t* pKey, point_t* pointArray )
{
	...
	NN_Assign ( pointArray[0].z, pkey->z, NUMWORDS );
	...
	for ( i = 0; i < S_MAX; i++ )
	{
		for ( j = 0; j<S_MAX+1; j++ )
		{
			if ( i == 0 && j==0 )
			{
				...
				NN_Assign ( pointArray[S_MAX].z, baseP->z, NUMWORDS );
			}
			else
				if ( j==0 )
				{
...
ecdsa_sign ( uint8_t* msg, uint16_t len, NN_DIGIT* r, NN_DIGIT* s, NN_DIGIT* d )
{
	...
	while ( !done )
	{
		...
		//calculating a third point P(u,v)
		...
		//}
		//printf("\nCrossed the if\n");
		//inverse of the random is calculated
		NN_ModInv ( k_inv, k, order, NUMWORDS );
		//printf("\nInverse of the random:%u\n",k_inv);
		//msg digest is calculated
		sha1_reset ( &ctx );
