/**
 * \defgroup x86 X86 specific implementation
 * \ingroup nn
 * @{
 */
...
NN_ModDivOpt (NN_DIGIT* a, NN_DIGIT* b, NN_DIGIT* c, NN_DIGIT* d, NN_UINT digits)
{
	...
	NN_DIGIT A[MAX_NN_DIGITS];
	NN_DIGIT B[MAX_NN_DIGITS];

	//NN_DIGIT V[MAX_NN_DIGITS];
	NN_DIGIT U[MAX_NN_DIGITS];
	NN_DIGIT Va[MAX_NN_DIGITS];
	...
	while ( ( tmp_even = NN_Cmp ( A, B, digits ) ) != 0 )
	{
		if ( NN_EVEN ( A, digits ) )
		{
			...
			if ( NN_EVEN ( B, digits ) )
			{
				...
				if ( NN_EVEN ( Va, digits ) )
				{
					NN_RShift ( Va, Va, 1, digits );
				}
				else
				{
					NN_Add ( Va, Va, d, digits );
					NN_RShift ( Va, Va, 1, digits );
				}
			}
			else
				if ( tmp_even > 0 )
				{
					...
					if ( NN_Cmp ( U, Va, digits ) < 0 )
					{
						NN_Add ( U, U, d, digits );
					}

					NN_Sub ( U, U, Va, digits );
				...
				else
				{
					...
					if ( NN_Cmp ( Va, U, digits ) < 0 )
					{
						NN_Add ( Va, Va, d, digits );
					}

					NN_Sub ( Va, Va, U, digits );
